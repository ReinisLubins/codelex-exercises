...
public class StreamsExercise {
    ...
    public static List<String> getFirstNames(List<String> names) {
        return names.stream().map((String n) -> n.split(" ")[0]).toList();
    }
    ...
    public static boolean anyMatch(List<User> users, int age) {
        return users.stream().anyMatch((User u) -> u.getAge() == age);
    }

    public static boolean noneMatch(List<User> users, int age) {
        return users.stream().noneMatch((User u) -> u.getAge() == age);
    }

    public static Optional<User> findAny(List<User> users, String name) {
        return users.stream().filter((User u) -> u.getName().equals(name)).findAny();
    }
    ...
    public static List<Integer> generate10RandomNumbers() {
        return new Random().ints(10, Integer.MIN_VALUE, Integer.MAX_VALUE).boxed().toList();
    }
